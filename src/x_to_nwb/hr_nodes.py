...
def getAmplifierGain(byte):
    ...
    return getFromList(
        [
            1e-3 / 1e-12 * x
            for x in [0.005, 0.010, 0.020, 0.050, 0.1, 0.2, 0.5, 1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000]
        ],
        byte,
    )
...
def convertDataFormatToNP(dataFormat):
    ...
    d = {"int16": np.int16, "int32": np.int32, "real32": np.float16, "real64": np.float32}
...
class AmplifierFile(TreeNode):
    ...
    rectypes = [None, AmplifierSeriesRecord, AmplifierStateRecord]
...
class Analysis(TreeNode):
    ...
    rectypes = [None, AnalysisMethodRecord, AnalysisFunctionRecord]
